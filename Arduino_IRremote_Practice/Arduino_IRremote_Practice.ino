...
//三星电源
uint16_t samsungCommand1 = 0x2;
//三星输入
uint16_t samsungCommand2 = 0x2;
//三星+
uint16_t samsungCommand3 = 0x7;
//三星-
uint16_t samsungCommand4 = 0xB;
//三星ok
uint16_t samsungCommand5 = 0x68;
uint8_t sRepeats = 3;

//空调开
unsigned int buf5[291] = 
{ 
...
void loop() 
{
    ...
    if (Serial.available() > 0) 
    {
        ...
        */
        ...
        if (inByte == 49)
        {
            IrSender.sendSamsung(samsungAddress, samsungCommand1, sRepeats);
        }
        //按2是输入选择
        if (inByte == 50)
        {
            IrSender.sendSamsung(samsungAddress, samsungCommand2, sRepeats);
        }
        //按3是音量+
        if (inByte == 51)
        {
            IrSender.sendSamsung(samsungAddress, samsungCommand3, sRepeats);
        }
        //按4是音量-
        if (inByte == 52)
        {
            IrSender.sendSamsung(samsungAddress, samsungCommand4, sRepeats);
        }
        //按5是空调开
        if (inByte == 53)
        {
